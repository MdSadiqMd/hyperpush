...
`tiny-cluster/` is one of the three equal canonical clustered surfaces in this repository alongside the generated `meshc init --clustered` scaffold and `cluster-proof/`. It is the smallest repo-owned package that still teaches the route-free source-first clustered contract.
...
This directory is intentionally the smallest route-free proof package, not a separate operator model. Keep it aligned with the generated scaffold and `cluster-proof/`, and treat package-owned control routes as drift rather than documentation variation.
