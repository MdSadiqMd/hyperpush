...
The hosted state is now narrower but still not milestone-green. The active rollout target is the workflow-only synthetic commit `e59f18203a30951af5288791bf9aed5b53a24a2a`, and the refreshed remote-evidence freshness gates in `scripts/verify-m034-s05.sh` plus `.tmp/m034-s09/rollout/workflow-status.json` confirm that `authoritative-verification.yml` is green on that exact `headSha` while `release.yml` is still red on the same SHA. That split matters: the authoritative live proof is now carrying the repaired registry latest-version semantics, but the release tag lane is still not exercising the full local S10 code surface, so the Windows staged installer smoke step still fails with `installed meshc.exe build installer smoke fixture failed`.

M034 is therefore in a closeout state where rollout freshness is honest, the authoritative hosted proof is green on the current remote target, and the remaining blocker is concentrated in the release tag path. The next slice must either roll the full S10 content onto the tag lane or finish the remaining Windows staged-smoke remediation before claiming a first-green hosted bundle; `.tmp/m034-s06/evidence/first-green/` must remain absent until that happens.
