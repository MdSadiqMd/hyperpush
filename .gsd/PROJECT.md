...
The remaining near-term trust gap is still hosted rollout and live public freshness, but S07 sharpened it further: the public installer/docs/packages contract is now centralized in `scripts/lib/m034_public_surface_contract.py`, the local S05/S07 workflow guards still pass, and the helper now uses curl-backed live fetches so `public-http` measures real public drift instead of failing on local TLS trust. Remote rollout is still incomplete, though: `origin/main` has only advanced to `8d3e76a65986e7bd5e00d107f9e11c1923ecd0ab`, the latest green `deploy.yml` run on that SHA still lacks the required `Verify public docs contract` step, the remote default branch still does not expose `authoritative-verification.yml`, there are still no hosted push runs for `release.yml`, `deploy-services.yml`, or `publish-extension.yml` on `v0.1.0` / `ext-v0.3.0`, and direct `public-http` checks now confirm that `meshlang.dev` itself is still serving stale installers/docs even while the packages-site and registry surfaces remain current.
