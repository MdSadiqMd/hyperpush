...
expected = {
    'owner-loss-recovery': {
        'continuity-api-owner-loss-recovery-': [
            'membership-node-a.http',
            'membership-node-a.json',
            'membership-node-b.http',
            'membership-node-b.json',
            'pre-loss-owner-status.http',
            'pre-loss-owner-status.json',
            'degraded-membership-node-b.http',
            'degraded-membership-node-b.json',
            'owner-lost-status.http',
            'owner-lost-status.json',
            'retry-rollover.http',
            'retry-rollover.json',
            'retry-pending-status.http',
            'retry-pending-status.json',
            'retry-completed-status.http',
            'retry-completed-status.json',
    ...
    'rejoin-truth': {
        'continuity-api-owner-loss-rejoin-': [
            'membership-node-a-run1.http',
            'membership-node-a-run1.json',
            'membership-node-b-run1.http',
            'membership-node-b-run1.json',
            'pre-loss-owner-status.http',
            'pre-loss-owner-status.json',
            'degraded-membership-node-b.http',
            'degraded-membership-node-b.json',
            ...
            'retry-completed-status.http',
            'retry-completed-status.json',
            'membership-node-a-run2.http',
            'membership-node-a-run2.json',
            'membership-node-b-run2.http',
            'membership-node-b-run2.json',
            'post-rejoin-node-a-status.http',
            'post-rejoin-node-a-status.json',
            'post-rejoin-node-b-status.http',
            'post-rejoin-node-b-status.json',
            'stale-completion-guard.http',
            'stale-completion-guard.json',
            'node-a-run1.stdout.log',
            'node-a-run1.stderr.log',
            'node-a-run2.stdout.log',
            'node-a-run2.stderr.log',
            'node-b-run1.stdout.log',
            'node-b-run1.stderr.log',
        ],
    },
    ...
    for file_name in required_files:
        ...
        if path.suffix == '.json':
            if 'membership' in file_name:
                ...
                ensure_json_shape(path, status_required)
        else:
...
assert_file_exists s02-contract .tmp/m042-s02/verify/status.txt "S02 status" "$ARTIFACT_DIR/03-s02-contract.log"
cp .tmp/m042-s02/verify/phase-report.txt "$ARTIFACT_DIR/03-s02-phase-report.txt"
cp .tmp/m042-s02/verify/status.txt "$ARTIFACT_DIR/03-s02-status.txt"
assert_file_contains_regex s02-contract "$ARTIFACT_DIR/03-s02-phase-report.txt" '^mesh-rt\tpassed$' "S02 mesh-rt replay did not pass" "$ARTIFACT_DIR/03-s02-contract.log"
assert_file_contains_regex s02-contract "$ARTIFACT_DIR/03-s02-phase-report.txt" '^cluster-proof-tests\tpassed$' "S02 cluster-proof test replay did not pass" "$ARTIFACT_DIR/03-s02-contract.log"
assert_file_contains_regex s02-contract "$ARTIFACT_DIR/03-s02-phase-report.txt" '^s02-degraded-status\tpassed$' "S02 degraded-status phase did not pass" "$ARTIFACT_DIR/03-s02-contract.log"
assert_file_contains_regex s02-contract "$ARTIFACT_DIR/03-s02-status.txt" '^ok$' "S02 status must be ok" "$ARTIFACT_DIR/03-s02-contract.log"

S03_E2E_BEFORE="$ARTIFACT_DIR/04-s03-e2e.before.txt"
capture_s03_snapshot "$S03_E2E_BEFORE"
run_expect_success s03-e2e 04-s03-e2e yes 480 \
  cargo test -p meshc --test e2e_m042_s03 -- --nocapture
assert_file_contains_regex s03-e2e "$ARTIFACT_DIR/04-s03-e2e.log" 'test continuity_api_owner_loss_retry_rollover_survivor_status_is_truthful \.\.\. ok' "owner-loss recovery scenario did not pass in shared S03 replay" "$ARTIFACT_DIR/04-s03-e2e.log"
assert_file_contains_regex s03-e2e "$ARTIFACT_DIR/04-s03-e2e.log" 'test continuity_api_same_identity_rejoin_preserves_newer_attempt_truth \.\.\. ok' "rejoin truth scenario did not pass in shared S03 replay" "$ARTIFACT_DIR/04-s03-e2e.log"

record_phase owner-loss-recovery started
collect_phase_artifacts \
  owner-loss-recovery \
  owner-loss-recovery \
  "$S03_E2E_BEFORE" \
  "$ARTIFACT_DIR/04-owner-loss-recovery-artifacts" \
  "$ARTIFACT_DIR/04-owner-loss-recovery-artifacts.txt"
record_phase owner-loss-recovery passed

record_phase rejoin-truth started
collect_phase_artifacts \
  rejoin-truth \
  rejoin-truth \
  "$S03_E2E_BEFORE" \
  "$ARTIFACT_DIR/05-rejoin-truth-artifacts" \
  "$ARTIFACT_DIR/05-rejoin-truth-artifacts.txt"
record_phase rejoin-truth passed
...
assert_file_contains_regex verifier-status "$PHASE_REPORT_PATH" '^s03-e2e\tpassed$' "shared S03 e2e replay did not pass" "$ARTIFACT_DIR/full-contract.log"
assert_file_contains_regex verifier-status "$PHASE_REPORT_PATH" '^owner-loss-recovery\tpassed$' "owner-loss recovery artifact validation did not pass" "$ARTIFACT_DIR/full-contract.log"
assert_file_contains_regex verifier-status "$PHASE_REPORT_PATH" '^rejoin-truth\tpassed$' "rejoin truth artifact validation did not pass" "$ARTIFACT_DIR/full-contract.log"

echo "verify-m042-s03: ok"
