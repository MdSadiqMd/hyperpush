...
if completed.returncode != 0:
    raise SystemExit('could not resolve origin remote; set M053_S03_GH_REPO explicitly')
url = completed.stdout.strip()
match = re.search(r'github\.com[:/](?P<slug>[^/]+/[^/.]+?)(?:\.git)?$', url)
if not match:
    raise SystemExit(f'could not derive GitHub repo slug from origin remote: {url!r}')
...
run_preflight() {
  ...
  local repo_slug=""
  if ! repo_slug="$(resolve_repo_slug 2>&1)"; then
    printf '%s\n' "$repo_slug" >"$log_path"
  ...
  if ! python3 - "$log_path" "$GH_BIN" "$GIT_BIN" "$GH_REPO" <<'PY'
from pathlib import Path
import os
import shutil
import sys
...
messages.append(f'ok: repository -> {repo_slug}')
...
run_candidate_refs() {
  ...
  if ! python3 - "$ROOT_DIR" "$CANDIDATE_REFS_PATH" "$GH_REPO" >"$log_path" 2>&1 <<'PY'
from pathlib import Path
...
run_remote_evidence() {
  ...
  if ! python3 - "$ROOT_DIR" "$CANDIDATE_REFS_PATH" "$REMOTE_RUNS_PATH" "$VERIFY_ROOT" "$GH_REPO" "$GH_BIN" "$GIT_BIN" >"$log_path" 2>&1 <<'PY'
from pathlib import Path
from datetime import datetime, timezone
import json
import os
        ...
        if not isinstance(workflows, list) or len(workflows) != 3:
            ...
            for workflow in workflows:
                if not isinstance(workflow, dict):
                    ...
                    continue
                for required_field in ['workflowFile', 'requiredJobs', 'requiredSteps', 'expectedHeadSha', 'observedHeadSha', 'freshnessStatus']:
