...
PY
  then
    fail_phase "$phase" "missing or malformed per-node artifacts" "$ARTIFACT_DIR/${phase}.artifact-check.log" "$dest_root"
  fi
}
...
REMOTE_BEFORE="$ARTIFACT_DIR/04-s02-remote-route.before.txt"
capture_s02_snapshot "$REMOTE_BEFORE"
run_expect_success s02-remote-route 04-s02-remote-route yes 180 \
  cargo test -p meshc --test e2e_m039_s02 e2e_m039_s02_routes_work_to_peer_and_logs_execution -- --nocapture
collect_phase_artifacts \
  s02-remote-route \
  remote \
  "$REMOTE_BEFORE" \
  "$ARTIFACT_DIR/04-s02-remote-route-artifacts" \
  "$ARTIFACT_DIR/04-s02-remote-route-artifacts.txt"

SOLO_BEFORE="$ARTIFACT_DIR/05-s02-local-fallback.before.txt"
capture_s02_snapshot "$SOLO_BEFORE"
run_expect_success s02-local-fallback 05-s02-local-fallback yes 120 \
  cargo test -p meshc --test e2e_m039_s02 e2e_m039_s02_falls_back_locally_without_peers -- --nocapture
collect_phase_artifacts \
  s02-local-fallback \
  solo \
  "$SOLO_BEFORE" \
  "$ARTIFACT_DIR/05-s02-local-fallback-artifacts" \
  "$ARTIFACT_DIR/05-s02-local-fallback-artifacts.txt"

echo "verify-m039-s02: ok"
