...
fn assert_startup_record(record: &Value, node_name: &str, request_key: &str) {
    ...
    assert_eq!(route_free::required_str(record, "execution_node"), node_name);
    assert_eq!(route_free::required_str(record, "replica_node"), "");
    assert_eq!(route_free::required_str(record, "replica_status"), "unassigned");
...
fn assert_clustered_route_record(record: &Value, node_name: &str, request_key: &str) {
    ...
    assert_eq!(route_free::required_str(record, "replication_health"), "local_only");
...
fn m053_s01_postgres_staged_deploy_bundle_boots_serves_crud_and_answers_cluster_inspection() {
    ...
    assert_eq!(
        fs::read_to_string(&bundle.pointer_path)
            .unwrap_or_else(|error| panic!("failed to read {}: {error}", bundle.pointer_path.display()))
            .trim(),
    ...
    assert!(
        apply.combined.contains("[deploy-apply] schema ready table=todos index=idx_todos_created_at"),
    ...
    let runtime_config = deploy::default_runtime_config(deploy::PACKAGE_NAME, &database.database_url);
    ...
    let run_result = catch_unwind(AssertUnwindSafe(|| {
        ...
        assert_eq!(health["clustered_handler"].as_str(), Some(deploy::STARTUP_RUNTIME_NAME));
        assert_eq!(
        ...
        assert_eq!(route_free::required_str(&status["authority"], "cluster_role"), "primary");
        assert_eq!(route_free::required_u64(&status["authority"], "promotion_epoch"), 0);
        assert_eq!(route_free::required_str(&status["authority"], "replication_health"), "local_only");
        ...
        assert_eq!(
            route_free::count_records_for_runtime_name(&startup_list, deploy::LIST_ROUTE_RUNTIME_NAME),
            0,
        ...
        assert_startup_record(&startup_record["record"], &runtime_config.node_name, &startup_request_key);
        ...
        let startup_entries = route_free::diagnostic_entries_for_request(&diagnostics, &startup_request_key);
        ...
        assert!(
            startup_transitions.contains(&"startup_dispatch_window")
                || route_free::required_str(&startup_record["record"], "replica_status") == "unassigned"
        );
        ...
        assert!(
            smoke.combined.contains("[deploy-smoke] creating todo via POST"),
        ...
        assert!(
            smoke.combined.contains("[deploy-smoke] CRUD smoke passed id="),
...
fn m053_s01_postgres_stage_deploy_rejects_invalid_bundle_path() {
    ...
    assert!(
        run.combined.contains("[stage-deploy] bundle path exists but is not a directory:"),
