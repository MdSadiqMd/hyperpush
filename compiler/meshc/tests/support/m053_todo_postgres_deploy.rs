...
pub fn default_cluster_runtime_pair_for_primary_owned_startup(
    ...
    for _ in 0..64 {
        ...
        let primary = postgres::TodoRuntimeConfig {
            ...
            node_name: format!("{project_name}-primary@{}:{cluster_port}", route_free::LOOPBACK_V4),
        ...
        let standby = postgres::TodoRuntimeConfig {
            ...
            node_name: format!("{project_name}-standby@[{}]:{cluster_port}", route_free::LOOPBACK_V6),
            discovery_seed: "localhost".to_string(),
            cluster_port,
...
pub fn json_request_snapshot_for_node(
    ...
    let response = send_http_request(config.http_port, method, path, body).unwrap_or_else(|error| {
        panic!(
            "{method} {path} failed on {}: {error}",
            config.http_port
        )
    });
    json_response_snapshot(artifacts, name, &response, expected_status, context, secret_values)
}
...
pub fn wait_for_dual_node_cluster_status(
    ...
    let membership = vec![runtime.primary.node_name.clone(), runtime.standby.node_name.clone()];
...
pub fn wait_for_primary_owned_startup_selection(
    ...
    let primary_list = route_free::wait_for_continuity_list_matching(
        ...
        &runtime.primary.cluster_cookie,
        |json| mirrored_startup_record_matches(
...
pub fn assert_runtime_logs(
    logs: &postgres::StoppedTodoApp,
    config: &postgres::TodoRuntimeConfig,
) {
...
pub fn run_stage_deploy_script(
    ...
    command.current_dir(project_dir).arg(&stage_script).arg(bundle_dir);
...
pub fn inspect_staged_bundle(bundle_dir: &Path, artifacts: &Path) -> StagedBundle {
    ...
    write_artifact(&bundle.pointer_path, format!("{}\n", bundle.bundle_dir.display()));
...
pub fn run_staged_deploy_smoke_script(
    ...
    let mut command = Command::new("bash");
    command.current_dir(&bundle.bundle_dir).arg(&bundle.smoke_script_path);
...
fn startup_request_owns_primary(primary_node: &str, standby_node: &str) -> bool {
    ...
    let owner_index =
        (stable_hash_u64(&format!("request::{}", startup_request_key())) as usize) % membership.len();
