...
fn continuity_list_snapshot(artifacts: &Path, label: &str, node_name: &str, cookie: &str) -> Value {
...
fn wait_for_new_request_key_for_runtime_name(
    artifacts: &Path,
    label: &str,
    node_name: &str,
    ...
    let after_snapshot = route_free::wait_for_continuity_list_matching(
        artifacts,
        label,
        node_name,
        &format!("new continuity record for {runtime_name}"),
        cookie,
        |json| {
            route_free::new_request_keys_for_runtime_name(before_snapshot, json, runtime_name).len()
                == 1
        },
    );
    let new_keys = route_free::new_request_keys_for_runtime_name(
        before_snapshot,
        &after_snapshot,
        runtime_name,
    );
    assert_eq!(
        new_keys.len(),
        1,
        "expected exactly one new request key for {runtime_name}, got {:?} in {}",
        new_keys,
        after_snapshot
    );
    ...
    assert!(!route_free::required_str(record, "request_key").is_empty());
...
fn m047_s07_clustered_http_routes_two_node_end_to_end() {
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        let before_first_primary = continuity_list_snapshot(
            &artifacts,
            "continuity-before-first-primary",
            &primary_node,
            SHARED_COOKIE,
        );
        let before_first_standby = continuity_list_snapshot(
            &artifacts,
            "continuity-before-first-standby",
            &standby_node,
            SHARED_COOKIE,
        );
        assert_eq!(
            route_free::required_u64(&before_first_primary, "total_records"),
            0
        );
        assert_eq!(
            route_free::required_u64(&before_first_standby, "total_records"),
            0
        );

        let first_success = get_json_snapshot(
            &artifacts,
            "route-success-first",
            primary_http_port,
            "/todos",
        ...
        let (after_first_primary, first_request_key_primary) =
            wait_for_new_request_key_for_runtime_name(
                &artifacts,
                "continuity-after-first-primary",
                &primary_node,
                &before_first_primary,
                SUCCESS_RUNTIME_NAME,
                SHARED_COOKIE,
            );
        let (after_first_standby, first_request_key_standby) =
            wait_for_new_request_key_for_runtime_name(
                &artifacts,
                "continuity-after-first-standby",
                &standby_node,
                &before_first_standby,
                SUCCESS_RUNTIME_NAME,
                SHARED_COOKIE,
            );
        assert_eq!(first_request_key_primary, first_request_key_standby);
        assert!(first_request_key_primary.starts_with("http-route::Api.Todos.handle_list_todos::"));
        ...
        let second_success = get_json_snapshot(
            &artifacts,
        ...
        let (after_second_primary, second_request_key_primary) =
            wait_for_new_request_key_for_runtime_name(
                &artifacts,
                "continuity-after-second-primary",
                &primary_node,
                &after_first_primary,
                SUCCESS_RUNTIME_NAME,
                SHARED_COOKIE,
            );
        let (after_second_standby, second_request_key_standby) =
            wait_for_new_request_key_for_runtime_name(
                &artifacts,
                "continuity-after-second-standby",
                &standby_node,
                &after_first_standby,
                SUCCESS_RUNTIME_NAME,
                SHARED_COOKIE,
            );
        assert_eq!(second_request_key_primary, second_request_key_standby);
        assert_ne!(first_request_key_primary, second_request_key_primary);
        assert_eq!(
            route_free::count_records_for_runtime_name(&after_second_primary, SUCCESS_RUNTIME_NAME),
            2
        );
        assert_eq!(
            route_free::count_records_for_runtime_name(&after_second_standby, SUCCESS_RUNTIME_NAME),
            2
        );
        ...
        let rejected_response = get_json_snapshot(
            &artifacts,
        ...
        let (after_rejected_primary, rejected_request_key_primary) =
            wait_for_new_request_key_for_runtime_name(
                &artifacts,
                "continuity-after-rejected-primary",
                &primary_node,
                &after_second_primary,
                UNSUPPORTED_RUNTIME_NAME,
                SHARED_COOKIE,
            );
        let (after_rejected_standby, rejected_request_key_standby) =
            wait_for_new_request_key_for_runtime_name(
                &artifacts,
                "continuity-after-rejected-standby",
                &standby_node,
                &after_second_standby,
                UNSUPPORTED_RUNTIME_NAME,
                SHARED_COOKIE,
            );
        assert_eq!(rejected_request_key_primary, rejected_request_key_standby);
        assert_eq!(
            route_free::count_records_for_runtime_name(
                &after_rejected_primary,
                UNSUPPORTED_RUNTIME_NAME
            ),
            1
        );
    ...
    for required in [
        ...
        "continuity-before-first-primary.json",
        "continuity-before-first-standby.json",
        "continuity-after-first-primary.json",
        "continuity-after-first-standby.json",
        "continuity-first-completed-primary.json",
        "continuity-first-completed-standby.json",
        "continuity-after-second-primary.json",
        "continuity-after-second-standby.json",
        "continuity-second-completed-primary.json",
        "continuity-second-completed-standby.json",
        "continuity-after-rejected-primary.json",
        "continuity-after-rejected-standby.json",
