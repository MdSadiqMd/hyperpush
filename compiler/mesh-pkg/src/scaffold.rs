...
Apply the staged schema, boot the staged binary, then smoke the running starter:
...
The staged bundle is the public deploy contract. Hosted runtimes or managed PostgreSQL providers can sit behind the same bundle later, but they are not part of this starter's required surface.
...
mod tests {
    ...
    fn m049_s01_postgres_scaffold_omits_sqlite_markers_and_secret_leaks() {
        ...
        let stage_script = std::fs::read_to_string(project_dir.join("scripts/stage-deploy.sh"))
            .unwrap();
        let apply_script =
