...
`cluster-proof/` is one of the three equal canonical clustered surfaces in this repository alongside the generated `meshc init --clustered` scaffold and `tiny-cluster/`. It keeps the same route-free source-first contract while carrying the deeper packaged failover/operator runbook.
...
- `cluster-proof/Dockerfile` is a two-stage build that copies only the built `cluster-proof` binary into the runtime image
- `cluster-proof/fly.toml` keeps only the process/build settings needed to run the binary in a Fly machine cluster
...
This directory is intentionally the deeper packaged failover/operator runbook, not a separate clustered contract. Keep it aligned with the generated scaffold and `tiny-cluster/`, and treat package-owned control routes as drift rather than documentation variation.
