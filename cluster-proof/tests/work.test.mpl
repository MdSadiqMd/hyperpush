...
describe("cluster-proof package smoke") do
  ...
  test("manifest and source stay source-first and route-free") do
    let manifest = read_required_file("cluster-proof/mesh.toml", "mesh.toml", "../mesh.toml")
    let main_source = read_required_file("cluster-proof/main.mpl", "main.mpl", "../main.mpl")
    let work_source = read_required_file("cluster-proof/work.mpl", "work.mpl", "../work.mpl")
  ...
  test("readme points operators to mesh-owned cli inspection") do
    let readme = read_required_file("cluster-proof/README.md", "README.md", "../README.md")
  ...
  test("packaging files stay honest about the route-free binary") do
    let dockerfile = read_required_file("cluster-proof/Dockerfile", "Dockerfile", "../Dockerfile")
    let fly_config = read_required_file("cluster-proof/fly.toml", "fly.toml", "../fly.toml")
    ...
    assert_contains(fly_config, "dockerfile = 'cluster-proof/Dockerfile'")
  ...
  test("obsolete helper files stay deleted") do
    assert(file_missing("cluster-proof/tests/config.test.mpl", "tests/config.test.mpl", "config.test.mpl"))
    assert(file_missing("cluster-proof/docker-entrypoint.sh", "docker-entrypoint.sh", "../docker-entrypoint.sh"))
    assert(file_missing("cluster-proof/cluster.mpl", "cluster.mpl", "../cluster.mpl"))
    assert(file_missing("cluster-proof/config.mpl", "config.mpl", "../config.mpl"))
    assert(file_missing(
